...
module soc_top #(
    ...
    core_multicycle #(.RESET_PC(RESET_PC)) u_core (
        ...
        .ext_mti(clint_mti), .ext_msi(clint_msi), .ext_mei(1'b0),
    ...
    assign dm_rsp_valid   = sram_b_rsp_valid | mmio_rsp_valid | clint_rsp_valid | dmem_bad_rsp_valid;
    assign dm_rsp_rdata   = sram_b_rsp_valid ? sram_b_rsp_rdata
                          : mmio_rsp_valid   ? mmio_rsp_rdata
                          : clint_rsp_valid  ? clint_rsp_rdata
                          :                    32'd0;
    assign dm_rsp_fault   = mmio_rsp_fault | clint_rsp_fault | dmem_bad_rsp_valid;
    ...
    always_ff @(posedge clk) begin
        ...
        else     dmem_bad_rsp_valid <= dm_req_valid && !dm_is_sram && !dm_is_mmio && !dm_is_clint;
    end
