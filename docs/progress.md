...
### Status
**Bitstream produced.** Post-route `axi_hello.bit` (2.09 MB) with WNS = −0.309 ns, TNS = −0.418 ns, 3 failing endpoints — all intra-MIG calibration FSM transitions in `u_ddr_phy_init` at the 166 MHz `clk_pll_i` domain. The user-side `soc_clk` domain (50 MHz — SoC, crossbar, protocol converter, UartLite) is clean at +0.194 ns WNS. No hold violations anywhere. memtest on silicon still pending.

### Two blockers the WIP entry below surfaced, and what each needed
1. **Router failure on `byte_lane_D`** — MIG 4.2 generates `PHY_0_BITLANES = 48'h3FF_3FE_FFF_B7B` for the Urbana pinout. Lane D's bit 0 (bit 36 of the 48-bit vector) = 1 on slot IOB_X1Y37 = M5 = `ddr3_reset_n`. `reset_n` is output-only (MIG drives it through its own OBUF, no SERDES), but BITLANES[36]=1 makes MIG's `byte_group_io` generate an `input_[0].iserdesdq` (ISERDESE2) + `output_[0].oserdes_dq_i` (OSERDESE2) anyway. Those land on no valid site, `PHASER_OUT_PHY_X1Y3`'s OCLK can't route to them, and the router dies with `[Route 35-7] Design has 4 unroutable pins`. Lane C's bit 24 = 0 because Y25 is genuinely unused, so MIG skips slot 0 there and the lane places cleanly — this is a MIG 4.2 regression; RealDigital's DDRtest reference uses MIG 4.0 with the same pinout and works.
   - **Fix:** post-generate regsub on the generated `fpga/build/axi_hello/ip/mig_ddr3_0/mig_ddr3_0/user_design/rtl/mig_ddr3_0_mig.v` — change `PHY_0_BITLANES = 48'h3FF_3FE_FFF_B7B` to `48'h3FE_3FE_FFF_B7B`, then `synth_ip`. Lives in `build_axi_hello.tcl` between `generate_target` and `synth_ip` for `mig_ddr3_0`.
2. **Timing failed catastrophically once the router succeeded** — first post-route WNS after the patch was −29.665 ns with 9677 failing endpoints, because the SoC was clocked directly on `ui_clk` (166.7 MHz from PHY 2:1). The multicycle core's EX stage is a 36 ns / ~57-logic-level path (instr → ALU/imm/branch → pc_q); it can't close at 166 MHz, and even with the pipelined core the shared `ui_clk` is too tight once a crossbar + protocol converter + SRAM + UART sit in the same domain.
   - **Fix:** added a third MMCM output (CLKOUT2 = 50 MHz → `soc_clk`), generated an `axi_clock_converter_0` IP, and moved `soc_top` / `axi_crossbar_0` / `axi_protocol_converter_0` / `axi_uartlite_0` onto `soc_clk`. The clock converter sits between the crossbar's M00 and `u_mig.s_axi`, crossing the 50 → 166 MHz boundary. Combined with switching axi_hello to the pipelined core by default (`CORE ?= pipeline` in the Makefile) and a narrowly-scoped `set_false_path -to [get_pins soc_rst_sync_reg[0]/D]` in the XDC for the async reset CDC, post-route WNS converged to −0.309 ns.

### What shipped
- **`fpga/ip/mig_urbana.prj`**. MIG7 project file for on-board ISSI IS43TR16640CL-125JBL (1Gbit, 64M×16 DDR3L 1.35V, modelled as `MT41K64M16XX-125`). 13×10×3 = 128 MB. Pin assignments verbatim from the RealDigital master XDC, all HR bank 34 — SSTL135 / DIFF_SSTL135, no DCI, internal VREF. `<SystemClock>No Buffer` + `<ReferenceClock>No Buffer` so the user RTL owns the MMCM in front of MIG (see below). PHY ratio 2:1, 666 MT/s.
- **`rtl/fpga/axi_hello_top.sv`**. Heavy rewrite:
  - Single user-owned `MMCME2_BASE` turns 100 MHz C1/B1 into CLKOUT0=333.333 MHz (MIG sys_clk_i), CLKOUT1=200 MHz (MIG clk_ref_i for IDELAYCTRL), CLKOUT2=50 MHz (soc_clk). No buffer-type games on MIG — it takes pre-BUFG'd nets.
  - `alive_clk` (N15 100 MHz, BUFG only) drives a heartbeat LED + reset synchronizer — proves the FPGA is configured even if MIG never calibrates.
  - `axi_clock_converter_0` bridges crossbar M00 (soc_clk) → MIG.s_axi (ui_clk). ID_WIDTH=0 because the crossbar drops IDs (NUM_SI=1, in-order responses).
  - `soc_rst_sync` is a 3-stage ASYNC_REG synchronizer that gates on rst_n (BTN0), both MMCM lock signals, `init_calib_complete`, and `~ui_clk_sync_rst`.
- **`fpga/scripts/build_axi_hello.tcl`**. Added `axi_clock_converter:2.1` IP block (AXI4, 32-bit data, ID_WIDTH=0, ACLK_ASYNC=1). UartLite `C_S_AXI_ACLK_FREQ_HZ` dropped to 50 MHz to match soc_clk. The MIG BITLANES regsub runs between `generate_target` and `synth_ip`, with a fallback warning if the string has shifted in a future MIG version.
- **`fpga/scripts/rebuild_pipeline.tcl`** (new). Lightweight rebuild that skips IP regeneration — useful when only the top-level RTL or XDC changes. Honours `USE_PIPELINE_CORE` like the full flow.
- **`fpga/constraints/urbana_axi_hello.xdc`**. Narrow `set_false_path -to [get_pins soc_rst_sync_reg[0]/D]` so the async reset CDC doesn't ride against the 166 → 50 MHz inter-clock arrival. Scoped tightly so the axi_clock_converter's own internal XDC constraints still apply.
- **`fpga/axi_hello/Makefile`**. `CORE ?= pipeline` — multicycle still exists as an opt-in for anyone who wants to try.
- **`sw/tests/c/memtest.c`** (from the WIP entry). Still built and ready; first silicon run pending.

### Tests
- **Sim regression unchanged** (74/76 both cores, 7/7 C tests). The behavioural Stage 7a router still drives the sim path — MIG can't run in Verilator.
