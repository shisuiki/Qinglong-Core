...
#   make -C sw -f riscv-tests.mk          # build rv32ui/um/ua/mi/si -p- tests
...
# Families in scope for RV32IMA + Sv32 MMU. rv32si uses env/p (no MMU) —
# it exercises S-mode CSRs, delegation, sbreak/scall, and wfi; the
# `dirty` test does enable Sv32 translation to probe A/D-bit handling.
# The -v- (virtual-memory) variants still fail because the base rv32
# multilib doesn't ship newlib headers that env/v/*.c uses.
...
list:
	@cd $(ISA_DIR) && ls 2>/dev/null \
	    | grep -E '^(rv32ui|rv32um|rv32ua|rv32mi|rv32si)-p-[A-Za-z0-9_-]+$$' \
	    | sort
...
help:
	@echo "Targets:"
	@echo "  all / isa   Build rv32ui/um/ua/mi/si -p- ELFs and .dumps"
