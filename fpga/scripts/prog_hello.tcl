# -----------------------------------------------------------------------------
# prog_hello.tcl
# Program the SP701 (xc7s100) with hello.bit over JTAG.
# -----------------------------------------------------------------------------
...
if {[info exists ::env(BITSTREAM)]} {
    ...
    set bitstream  [file normalize [file join $script_dir .. build hello hello.bit]]
}

if {![file exists $bitstream]} {
    puts "ERROR: bitstream not found: $bitstream"
    puts "       run `make synth` first or set BITSTREAM=/path/to/hello.bit"
    exit 1
}

puts "==> prog_hello: bitstream = $bitstream"
...
foreach dev $all_devices {
    ...
    puts "==> prog_hello: found device: $dev  (part=$dev_name)"
